...
public class TcpServer {
	...
	public TcpServer(int port, int MaxClients) throws ClientThreadsException, ServerException {
		try {
			this.port = port;
	...
	public void onAccept(AcceptListener listener) throws ClientThreadsException, ServerException {
		try {
			while (true) {
				threads.bind(server.accept(), listener);
			}
		} catch (IOException e) {
...
class ClientThread extends Thread {
	...
	public void run() {
		...
		if (modes.length > 30) {
			...
			try {
				throw new ClientThreadsException("Too many args up to 30 (client): " + this.num); // 30
																									// will
																									// lag
																									// and
																									// be
